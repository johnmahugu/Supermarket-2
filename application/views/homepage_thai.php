...
<body>
		...
		<div class="container">
			...
			<div class="row">
				<div class="col-xs-12">
					...
					<span>แพ็คเกจท่องเที่ยวในประเทศและต่างประเทศ<br>
					<p style="font-size:20px;color:#ffff00;margin-top:-15px;">ที่นักท่องเที่ยวนิยมเข้ามาเลือกซื้อมากที่สุด</p></span><br></h1>
				</div>
			</div>
		</div>
	</header>
	...
	<div class="container">
		<div class="row">
			<a href="<?=base_url()?>thai-tour"><div class="col-xs-6 no-pd">
				<p class="choose-tours-box current">
					THAILAND DOMESTIC TOURS
				</p>
			</div></a>
			<a href="<?=base_url()?>international-tour"><div class="col-xs-6 no-pd">
			...
			<div class="col-md-4 col-sm-6">
				<div class="tour-box">
					<div class="img">
						...
						?>
						<span><?=$hilight_est_date[0];?> Day <?=$hilight_est_date[1];?> Night</span></p>
						</div>
					</div>
					<div class="tag">
						<p><?=$row['tour_type'];?></p>
	...
	function listPackage($base_url, data){
		...
		if($.trim(data['package'])){
			...
			for($i=0;$i<data['package'].length;$i++){
				...
				if(data['package'][$i].tour_currency == 'THB'){
					$result['list_package'] += '<p>Price: '+numeral(data['package'][$i].tour_startPrice).format('0,0')+' Baht<br>';
				}else{
					$result['list_package'] += '<p>Price: $'+numeral(data['package'][$i].tour_startPrice).format('0,0')+'<br>';
				}
				$est_dayNight = data['package'][$i].tour_dayNight.split(",");
				$result['list_package'] += '<span>'+$est_dayNight[0]+' Day '+$est_dayNight[1]+'</span></p>';
