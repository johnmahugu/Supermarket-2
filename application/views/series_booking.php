				...
				<div class="form-group">
					...
					<div class="form-group">
						...
						<div class="list" room-type="Twin / Tripple room">
							...
							<div class="col-sm-4 col-xs-7"><p class="priceroom"><?=number_format($package['tour_startPrice']);?></p><p>&nbsp;/ person</p></div>
						...
						<?php } ?>
							...
							<div class="col-sm-4 col-xs-7"><input type="hidden" value="<?=$row['tc_price'];?>"><p class="priceroom"><?=number_format($package['tour_startPrice']+$row['tc_price']);?></p><p>&nbsp;/ person</p></div>
		...
		if(!check_room()){
			...
			if($('select[name="daytrip"]').val() == null){
				...
				if($status_room && $status_day && $status_tourist){
					...
					$b_detail += '"total_amount":[{"amount":"'+$totalamount+'"}]';
					$b_detail += '}';
